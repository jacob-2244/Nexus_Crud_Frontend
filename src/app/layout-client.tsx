...
import Footer from "@/components/Footer";
import { ThemeProvider } from "@/components/Theme-provider";
import { isAuthPath } from "@/lib/routeGuard";
import { useMenu, MenuItem } from "@/hooks/useMenu";
import { isPathAllowed } from "@/lib/routeGuard";
...
export default function LayoutClient({ children }: { children: React.ReactNode }) {
  ...
  // Fetch dynamic menu from backend for current user role
  ...
  // Role-based routing: redirect if user's role cannot access this path
  useEffect(() => {
    if (!isLoading && !menuLoading && isAuthenticated && !isAuthPath(pathname) && menuItems.length) {
  ...
  // Show loading if auth or menu not ready
  if (isLoading || menuLoading || (!isAuthenticated && !isAuthPath(pathname))) {
    return (
      <div className="flex items-center justify-center min-h-screen">
        <div className="text-center">
          <div className="animate-spin rounded-full h-32 w-32 border-b-2 border-gray-900"></div>
